...
const perspectives = [
  { name: "Finance", question: "Does this change the investment case?", detail: "Margin · Return on capital · Payback", tag: "A financial lens" },
    ...
    <>
      ...
      <main id="main">
        ...
        <section className="approach-section" id="approach"><div className="container"><div className="section-heading"><div><p className="eyebrow">From measurement to meaning</p><h2>Run the disagreement.<br /><em>Before the commitment.</em></h2></div><p>A coherent formula is a starting point.<br />A useful metric also needs a shared understanding.</p></div><div className="steps-grid">
          ...
          <article className="step-card"><span className="step-number">02</span><span className="step-icon" aria-hidden="true">⇄</span><h3>Change the perspective.</h3><p>Map where finance, sustainability and operations diverge on meaning, evidence, timing and control.</p><span className="step-foot">Explore the conflict map</span></article>
        ...
        <FrameworkExplorer />
        <section className="lab-section" id="lab"><div className="container"><div className="section-heading lab-heading"><div><p className="eyebrow">The friction lab</p><h2>Same metric.<br /><em>Different decisions.</em></h2></div><div className="demo-explainer"><span className="demo-label">Working prototype</span><p>Choose an example or bring your own metric. Explore assumptions, compare role perspectives, and build a meeting agenda from the open issues.</p></div></div><MetricLab /><p className="lab-disclaimer">Agreement is a reason to investigate further, not proof that a metric is valid. Human judgement remains essential.</p></div></section>
        <section className="closing container"><p className="eyebrow">Our conviction</p><h2>The output is an agenda.<br /><em>The decision stays with you.</em></h2><p>A research framework helps structure the questions. Evidence makes the interpretation accountable. People decide what to do next.</p><a className="button primary" href="#lab">Start the conversation <span aria-hidden="true">↗</span></a></section>
      </main>
